...
Run with .venv/bin/python scripts/public_demo.py start|stop|status.
"""
...
def main():
    ...
    parser.add_argument("action", choices=["start", "stop", "status", "backup"])
    args = parser.parse_args()
    if not (DEMO / "instance.json").exists():
        raise SystemExit("Prepare the dedicated migrated demo instance first.")
    state = json.loads(STATE.read_text()) if STATE.exists() else {}
    if args.action == "backup":
        stop(state, ["tunnel", "api", "worker", "awake"])
        values = {
            **dotenv_values(DEMO / "migration.env"),
            **dotenv_values(DEMO / ".env"),
        }
    ...
    if args.action == "start":
        ...
        if any(key in values for key in ("DATABASE_ADMIN_URL", "POSTGRES_PASSWORD")):
